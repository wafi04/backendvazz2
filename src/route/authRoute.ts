...
import { registerSchema, loginSchema, updatePasswordSchema } from "../validation/user";
  ...
  async (c) => {
    try {
      ...
      return c.json({
        ...
        user: result.user,
        // Optional: include token for clients that prefer headers
        token: result.token
      });
    } catch (error) {
...
authRoutes.put(
  ...
  validator("json", (value, c) => {
    ...
    if (!value.username || !value.amount || !value.operation) {
      return createErrorResponse("Missing required fields: username, amount, operation", 400);
    }
  ...
  async (c) => {
    try {
      ...
      const newBalance = await authService.updateBalance(username, amount, operation);
